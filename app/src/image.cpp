			...
			for (int i = 0; i < width; i++)
				{
					...
					if (png_alpha != NULL)
						new_alpha[js * w + is] += static_cast<unsigned char> (
							png_alpha[ipos] / scale2);
					ipos++;
				}
		}
 ...
 */
void
Image::Merge (Image *background, const int x, const int y)
{
...
void
Image::Merge_non_crop (Image *background, const int x, const int y)
{
					...
					if (j >= y && i >= x && j < pnl_h_end && i < pnl_w_end)
						{
							for (int k = 0; k < 3; k++)
								{
									if (png_alpha != NULL)
										tmp = rgb_data[IMG_POS_RGB (pnl_pos,
																	k)]
 ...
 */
void
Image::Tile (const int w, const int h)
{
	...
	unsigned char *new_rgb
		= (unsigned char *)malloc (3 * newwidth * newheight);
					...
					for (int j = 0; j < height; j++)
						{
							for (int i = 0; i < width; i++)
								{
									opos = j * width + i;
									ipos = r * width * height * nx
										   + j * newwidth + c * width + i;
									for (int k = 0; k < 3; k++)
										{
											new_rgb[3 * ipos + k]
												= static_cast<unsigned char> (
					...
					if (j >= y && i >= x && j < y2 && i < x2)
						{
							...
							if (png_alpha != NULL)
								new_alpha[ipos] = static_cast<unsigned char> (
									png_alpha[opos]);
							ipos++;
						}
					opos++;
 ...
 */
void
Image::Center (const int w, const int h, const char *hex)
{
 ...
 */
void
Image::Plain (const int w, const int h, const char *hex)
{
...
void
Image::computeShift (unsigned long mask, unsigned char &left_shift,
...
Image::createPixmap (Display *dpy, int scr, Window win)
{
	...
	XImage *ximage = XCreateImage (dpy, visual, depth, ZPixmap, 0, pixmap_data,
								   width, height, 8, 0);
				...
				for (j = 0; j < height; j++)
					{
						for (i = 0; i < width; i++)
							{
								xc.red = (unsigned short)(rgb_data[ipos++]
														  & 0xe0);
								xc.green = (unsigned short)(rgb_data[ipos++]
															& 0xe0);
								xc.blue = (unsigned short)(rgb_data[ipos++]
														   & 0xc0);

								xc.pixel = xc.red | (xc.green >> 3)
										   | (xc.blue >> 6);
								XPutPixel (
									ximage, i, j,
				...
				for (j = 0; j < height; j++)
					{
						for (i = 0; i < width; i++)
							{
								red = (unsigned long)rgb_data[ipos++]
									  >> red_right_shift;
								green = (unsigned long)rgb_data[ipos++]
										>> green_right_shift;
								blue = (unsigned long)rgb_data[ipos++]
									   >> blue_right_shift;

								pixel = (((red << red_left_shift)
				 ...
				 unsigned char **rgb)
{
	...
	if (infile == NULL)
		{
			logStream << APPNAME << "Cannot fopen file: " << filename
					  << std::endl;
			return ret;
		}
	...
	*height = cinfo.output_height;
	...
	rgb[0] = (unsigned char *)malloc (3 * cinfo.output_width
									  * cinfo.output_height);
	if (rgb[0] == NULL)
		{
	...
	if (cinfo.output_components == 3)
	...
	else if (cinfo.output_components == 1)
		{
			...
			if (ptr == NULL)
				{
					logStream << APPNAME
							  << ": Can't allocate memory for JPEG file."
							  << std::endl;
					goto rgb_free;
				}
				...
				unsigned char **rgb, unsigned char **alpha)
{
	...
	if (infile == NULL)
		{
			logStream << APPNAME << "Can not fopen file: " << filename
					  << std::endl;
			return ret;
		}
	...
	if (color_type == PNG_COLOR_TYPE_RGB_ALPHA
		|| color_type == PNG_COLOR_TYPE_GRAY_ALPHA)
		{
			...
			if (alpha[0] == NULL)
				{
					logStream << APPNAME
							  << ": Can't allocate memory for alpha channel "
								 "in PNG file."
							  << std::endl;
					goto png_destroy;
				}
	...
	for (i = 0; i < *height; i++)
		{
			...
			if (row_pointers == NULL)
				{
					logStream << APPNAME
							  << ": Can't allocate memory for PNG file."
							  << std::endl;
					goto rows_free;
				}
		}
