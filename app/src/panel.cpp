...
Panel::Panel(Display *dpy, int scr, Window root, Cfg &config, const std::string &themedir, PanelType panel_mode)
	: mode(panel_mode), m_config_panel(config), m_window_root(root), m_display(dpy), m_screen(scr), session_name(""),
	  session_exec("")
{
	if (mode == Mode_Lock)
	{
...
void Panel::WrongPassword(int timeout)
{
	...
	ResetPasswd();
	OnExpose();
	/* The message should stay on the screen even after the password field is cleared, methinks. I don't like this
...
void Panel::Message(const std::string &text)
{
	...
	if (mode == Mode_Lock)
	{
		draw = XftDrawCreate(m_display, Win, DefaultVisual(m_display, m_screen), DefaultColormap(m_display, m_screen));
	}
	else
	{
...
std::string Panel::getSession() { return session_exec; }
...
void Panel::ResetName(void) { NameBuffer.clear(); }
...
const std::string &Panel::GetName(void) const { return NameBuffer; }

const std::string &Panel::GetPasswd(void) const { return PasswdBuffer; }
