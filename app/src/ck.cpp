...
namespace Ck
{
	...
	const char *
	Session::get_x11_device (const std::string & display)
	{
	...
	void
	Session::open_session (const std::string & display, uid_t uid)
	{
	...
	const char *
	Session::get_xdg_session_cookie ()
	{
...
std::ostream &
operator<< (std::ostream & os, const Ck::Exception & e)
{
	os << e.func << ": " << e.errstr;
	return os;
}
